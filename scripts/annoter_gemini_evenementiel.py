# -*- coding: utf-8 -*-
"""Annotation du corpus événementiel #MissFrance2026 par Gemini : 1 appel par tweet,
T=0, JSON forcé. Refus consignés sans relance, reprise en place, journal complet.
"""
...
FICHIER_SORTIE = SOURCES / f"annotations_gemini_evenementiel_passe{PASSE}.xlsx"
FICHIER_JOURNAL = SOURCES / f"journal_gemini_evenementiel_passe{PASSE}.json"


PRIX_ENTREE, PRIX_SORTIE = 1.50, 9.00


PROMPT = (DEPOT / "consignes" / "evenementiel.md").read_text(encoding="utf-8")
...
def charger_tweets():
    ...
    for row in ws.iter_rows(min_row=2, values_only=True):
        if row[0] is None or row[3] in (None, ""):
            continue
        n = int(row[0])
        tweets.append({"n": n, "id": str(row[1]), "texte": str(row[3]), "parent": ""})
    wb.close()
    return tweets
